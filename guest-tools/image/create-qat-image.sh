#!/bin/bash
#
# Create a Ubuntu EFI cloud QAT guest image. It can run on any Linux system with
# required tool installed like qemu-img, virt-customize, virt-install, etc. It is
# not required to run on a QAT capable system.
#
...
GUEST_IMG="qat-guest-ubuntu-24.04.qcow2"
SIZE=50
GUEST_USER=${GUEST_USER:-"ubuntu"}
GUEST_PASSWORD=${GUEST_PASSWORD:-"123456"}
GUEST_HOSTNAME=${GUEST_HOSTNAME:-"qat-guest"}
GUEST_REPO=""
...
Usage: $(basename "$0") [OPTION]...
  ...
  -n                        Guest host name, default is "qat-guest"
  -u                        Guest user name, default is "qat"
  -p                        Guest password, default is "123456"
  -s                        Specify the size of guest image
  -o <output file>          Specify the output file, default is qat-guest-ubuntu-24.04.qcow2.
...
EOT
    ...
    # boot the VM for cloud init configuration
    # the cloud init iso will be mounted in the VM as cdrom used as local data source
    # for cloud-init
    # cloud-init service will use it as a data source
    virt-install --memory 4096 --vcpus 4 --name qat-config-cloud-init \
        --disk /tmp/${GUEST_IMG} \
    ...
    virsh destroy qat-config-cloud-init || true
    virsh undefine qat-config-cloud-init || true
}
...
setup_guest_image() {
    ...
    virt-customize -a /tmp/${GUEST_IMG} \
        --copy-in ${CURR_DIR}/../setup-qat-guest.sh:/tmp/
    virt-customize -a /tmp/${GUEST_IMG} \
...
# sanity cleanup
virsh shutdown qat-config-cloud-init &> /dev/null
sleep 1
virsh destroy qat-config-cloud-init &> /dev/null
virsh undefine qat-config-cloud-init &> /dev/null
...
virsh shutdown qat-config-cloud-init > /dev/null 2>&1
sleep 1
virsh destroy qat-config-cloud-init  > /dev/null 2>&1
virsh undefine qat-config-cloud-init > /dev/null 2>&1
...
virt-customize -a /tmp/${GUEST_IMG} --root-password password:root
...
ok "QAT guest image : ${WORK_DIR}/${GUEST_IMG}"
